...
<form method="post" action="{{ route('prescription.store') }}">
   ...
   <div class="row justify-content-center">
      ...
      <div class="col-md-8">
         <div class="card shadow mb-4">
            <div class="card-header py-3">
               <h6 class="m-0 font-weight-bold text-primary">{{ __('sentence.Long Term Diseas') }}</h6>
            </div>
            <div class="card-body">
               <fieldset class="illness_labels">
                  <div class="repeatable"></div>
                  <div class="form-group">
                     <a type="button" class="btn btn-success add text-white" align="center"><i class='fa fa-plus'></i> {{ __('sentence.Add diseas') }}</a>
                  </div>
               </fieldset>
            </div>
         </div>
...
<script type="text/template" id="illness_labels">
 ...
 <select class="form-control select2 select2-hidden-accessible" name="name_illness[]" id="illness" tabindex="-2" aria-hidden="true">
                                   <option value="">{{ __('sentence.Select diseas') }}...</option>
                                   @foreach($illness as $illnesses)
                                       <option value="{{ $illnesses->id }}">{{ $illnesses->name_illness }}</option>
                                   @endforeach
                                 </select>
                             </div>
                        </div>
